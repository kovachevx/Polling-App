...
const SinglePollCard = ({ options }) => {
    ...
    return (
        <div className={classes.singlePollContainer} key={options.id}  >
            ...
            <div>
                {(isLoggedIn && options.voters.includes(loggedUser.id)) && <p>You've already voted!</p>}
                {(isLoggedIn && !options.voters.includes(loggedUser.id)) && <Button className={classes.btn} id={options.id} color='primary' onClick={viewPollHandler}>Vote</Button>}
